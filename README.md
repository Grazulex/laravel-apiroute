...
- **Endpoint deprecation** - `#[Deprecated]` attribute or `->deprecated()` route macro with RFC 8594 / RFC 9745 headers and 410 sunset policy
...
| Header | When | Notes |
| --- | --- | --- |
| `Deprecation` | `since` is set | RFC 8594 date |
| `Sunset` | `sunset` is set | RFC 8594 date |
...
`successor` is resolved in this order: a named route, a path starting with `/`, or an absolute URL.
